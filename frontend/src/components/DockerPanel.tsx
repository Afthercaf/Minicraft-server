...
interface DockerInfo { available: boolean; container: string; image?: string; status: string; running?: boolean; startedAt?: string; health?: string; restartPolicy?: string; cpu?: string; memory?: string; network?: string }
export default function DockerPanel() {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[['CPU',info?.cpu],['Memoria',info?.memory],['Red',info?.network],['Salud',info?.health||'Sin healthcheck']].map(([a,b])=><div className="panel" key={a}><p className="text-xs uppercase tracking-wider text-slate-500">{a}</p><p className="mt-3 text-xl font-semibold text-sky-400">{b||'—'}</p></div>)}</div>
